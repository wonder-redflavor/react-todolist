...
function DetailBox() {
  ...
  // const filteredTodos =
  //   useSelector((state) => {
  //   // console.log("state.todos: ", state.todos.id);
  //   // console.log("state.todos , params.id: ", params.id);
  //   return state.todos.filter((item) => item.id === params.id);
  // });

  const filteredTodos = todos.todos.filter((item) => item.id === params.id);
  ...
  return (
    <StyledDiv>
      ...
      <StyledButton onClick={handleButtonClick}> 수정하기</StyledButton>
    </StyledDiv>
  );
}
